...
const allRepoController = async (req: Request, res: Response) => {
  try {
    ...
    const userNameInput = userNameRaw.trim().toLowerCase();
    
    // Check if fetching all public repos (special case: "*")
    const fetchAllPublic = userNameInput === "*";
    ...
    // determine if requester is the owner (only for single-user case)
    const requester = req.user;
    const isOwner = !fetchAllPublic && requester && typeof requester.userName === "string" && requester.userName === userNameInput;

    // build query
    const base: any = {};
    if (fetchAllPublic) {
      // Fetch all public repos from all users
      base.visibility = "public";
    } else {
      // Fetch repos for specific user
      base.userName = userNameInput;
      if (!isOwner) {
        base.visibility = "public";
      }
    }
